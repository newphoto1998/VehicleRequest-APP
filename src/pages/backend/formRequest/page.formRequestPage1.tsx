...
function FormRequestPage1() {
  ...
  const District = async (province_id: string) => {
    ...
    if (province_id == "default") {
      ...
      defaultOnSelect("province")
        ...
        await handleChange("cusAddr_Province",province.filter((x: Province) => x.id == Number(province_id))[0].name_th);
        settambonFilter([]);
  ...
  const Zipcode = async (tambon_id: string) => {
    ...
    if(tambon_id == "default"){
        ...
        handleChange("cusAddr_Tambon",tambonFilter.filter((x: Tambon) => x.id == Number(tambon_id))[0].name_th + " " + zipcodes[0].zip_code);
  ...
  const validate = () => {
    const newErrors = {};
    console.log(reqeustFormBase.busRoute_Address)
  ...
  async function handleChange(key: string, value: any ) {
    if (key == "busRoute_Address") {
      ...
      dispatch({
        type: "ADD_DATA_IN_FORM",
        payload: { ...reqeustFormBase, busRoute_Address_Data: value },
      });
      ...
      dispatch({
        type: "ADD_DATA_IN_FORM",
        payload: { ...reqeustFormBase, busRoute_Employee_Data: value },
      });
      ...
      <form onSubmit={handleSubmit}>
              ...
              <div>
                <input
                  ...
                  className={`bg-[#FAFFB3] text-gray-700 text-sm p-2 
                    ...
                    `}
                />
                  {errors.busRoute_Address_Data  && <p className="mt-1 text-sm text-red-500">{errors.busRoute_Address_Data}</p>}
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex justify-start gap-10">
            ...
            {busRouteEmployee == "true" && (
              <div>
                ...
                />
                {errors.busRoute_Employee_Data  && <p className="mt-1 text-sm text-red-500">{errors.busRoute_Employee_Data}</p>}
        ...
        <div className="grid md:grid-cols-4 gap-3 mt-8 [&>*>*]:bg-[#FAFFB3] ">
          ...
          <div>
            ...
            <select value={districtSelected} onChange={(e) => {Tambon(e.target.value)}} id="countries"
              className=" border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            >
              <option value={"default"} selected>อำเภอ</option>
          ...
          <div>
            <select value={tambonSelected} onChange={(e) => {Zipcode(e.target.value);}}
              id="countries"
              className=" border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            >
              <option value={"default"} selected>ตำบล</option>
